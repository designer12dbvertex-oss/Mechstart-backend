...
export const Login = async (req, res, next) => {
  try {
    ...
    const emailTrimmed = email.trim().toLowerCase(); // ⭐ IMPORTANT
    const isExistEmail = await AdminModel.findOne({ email: emailTrimmed });
    if (!isExistEmail)
      return res
        .status(404)
...
export const upsertLeadership = async (req, res) => {
  try {
    ...
    const parsedSubPoints1 =
      typeof subPoints1 === 'string' ? JSON.parse(subPoints1) : subPoints1;
    const parsedSubPoints2 =
      typeof subPoints2 === 'string' ? JSON.parse(subPoints2) : subPoints2;
    const parsedFunctionSubPoints =
      typeof functionSubPoints === 'string'
        ? JSON.parse(functionSubPoints)
        : functionSubPoints;
    ...
    if (existing) {
      ...
      if (files.banner)
        updateData.banner = `public/uploads/${files.banner[0].filename}`;
      if (files.image1)
        updateData.image1 = `public/uploads/${files.image1[0].filename}`;
      if (files.image2)
        updateData.image2 = `public/uploads/${files.image2[0].filename}`;
      if (files.functionImage)
        updateData.functionImage = `public/uploads/${files.functionImage[0].filename}`;
    }

    let saved;

    if (existing) {
      saved = await LeadershipModel.findByIdAndUpdate(
        existing._id,
        updateData,
        { new: true }
      );
  ...
  try {
    ...
    const deleteOldFile = (oldPath) => {
      ...
      const fullPath = path.join(
        'public/uploads',
        oldPath.replace('public/uploads/', '')
      );
      if (fs.existsSync(fullPath)) {
...
export const createScalingStrategy = async (req, res, next) => {
  try {
    ...
    if (!title || !description) {
      return res
        .status(400)
        .json({ success: true, message: 'All field are rewuired' });
    }
    if (!req.file)
      return res
        .status(400)
...
export const deleteScalingStrategy = async (req, res, next) => {
  try {
    ...
    let filePath;
    if (existing.image) filePath = path.join(process.cwd(), existing.image);
